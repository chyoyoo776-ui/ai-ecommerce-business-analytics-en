...
# Avoid defaulting multiselects to every value, because Streamlit renders each
# selected value as a chip and a long brand list can consume the entire screen.
with st.container(border=True):
    st.markdown("#### Filters")
    ...
    with top_right:
        st.caption("Default view uses all categories, all brands, and all campaign types to keep the page compact.")

    col1, col2, col3 = st.columns(3)

    with col1:
        category_options = sorted(sku["category"].unique().tolist())
        use_all_categories = st.checkbox("All categories", value=True)
        if use_all_categories:
            category_sel = category_options
            st.caption(f"{len(category_sel)} categories selected")
        else:
            category_sel = st.multiselect("Choose categories", category_options, default=category_options[:1])

    brand_options = sorted(sku[sku["category"].isin(category_sel)]["brand"].unique().tolist()) if category_sel else []

    with col2:
        brand_mode = st.selectbox("Brand", ["All brands", "Single brand", "Custom brand group"])
        if brand_mode == "All brands":
            brand_sel = brand_options
            st.caption(f"{len(brand_sel)} brands selected")
        elif brand_mode == "Single brand":
            selected_brand = st.selectbox("Choose brand", brand_options) if brand_options else None
            brand_sel = [selected_brand] if selected_brand else []
        else:
            with st.expander("Choose brands", expanded=True):
                brand_sel = st.multiselect("Brand list", brand_options, default=brand_options[:5])
            st.caption(f"{len(brand_sel)} of {len(brand_options)} brands selected")

    with col3:
        campaign_type_options = sorted(sku["campaign_type"].unique().tolist())
        use_all_campaign_types = st.checkbox("All campaign types", value=True)
        if use_all_campaign_types:
            campaign_type_sel = campaign_type_options
            st.caption(f"{len(campaign_type_sel)} campaign types selected")
        else:
            campaign_type_sel = st.multiselect(
                "Choose campaign types",
                campaign_type_options,
                default=campaign_type_options[:1],
            )
