...
with col_f2:
    channel_options = df["channel"].unique().tolist()
    channels_sel = st.multiselect("Filter channel", options=channel_options, default=channel_options)
...
with col_b:
    ...
    ch_agg["conversion_rate"] = fdf.groupby("channel").apply(
        lambda x: x["orders"].sum() / x["traffic"].sum(), include_groups=False
    ).values
